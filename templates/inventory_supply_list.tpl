{if !empty($inv_supply_list) || !empty($lub_supply_list)}
...
<table  border="0" cellspacing="0" cellpadding="1" id="outer-table">
	...
	<td>
		...
		{if !empty($inv_supply_list)}
            ...
            <td id="horizontal-spacer">{$inv_supply_list[c]->getSupplyDate()}</td>
            {* <td id="horizontal-spacer">{$inv_supply_list[c]->getNiapNumber()}</td> *}
            <td id="horizontal-spacer">{$inv_supply_list[c]->getPlateNumber()}</td>
            <td id="horizontal-spacer">{$inv_supply_list[c]->getDeliveryOrderNumber()}</td>
            <td id="horizontal-spacer">{$inv_supply_list[c]->getSupplyValue()}
                {if !empty($inv_types)}
	             {section name=d loop=$inv_types}
	               {if $inv_supply_list[c]->getInvType() == $inv_types[d]->getInvType()}
	                 ltr
			...
			<tr><td colspan="7" id="horizontal-spacer"><sup>{$paginatedGasLinks}</sup></tr>
			<tr><td colspan="7" id="vertical-spacer"><img src="images/dot.gif" border="0"/></td></tr>
			</table>
...
{if !empty($lub_supply_list)}
  ...
  {/if}
     {if !empty($inv_supply_list) || !empty($lub_supply_list)}
        ...
        <table  border="0" cellspacing="0" cellpadding="0" id="inventory-supply" height="24">
          <tr>
            <td align="right"><a id="inv_supply_delete_all" href="InventorySupplyAction.php?method=deleteAll{php} echo ("&amp;".strip_tags(SID));{/php}" onclick="return confirm('Anda ingin menghapus semua data?');" class="button">Hapus Semua</a></td>
          </tr>          
        </table>
       {/if}
     {/if}
